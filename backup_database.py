...
from xelbot.database.models import (
    GameSession,
    JeopardyQuestion,
    TurnoverUsage,
    get_session,
)
from xelbot.utils.logger import setup_logger
...
class DatabaseBackup:
    ...
    def _parse_database_url(self) -> Dict[str, str]:
        ...
        if parsed.path:
            database_name = (
                str(parsed.path)[1:]
                if str(parsed.path).startswith("/")
                else str(parsed.path)
            )
    ...
    def create_sql_backup(self, filename: Optional[str] = None) -> str:
        ...
        try:
            ...
            with open(backup_path, "w", encoding="utf-8") as f:
                ...
                for q in questions:
                    ...
                    air_date_val = (
                        "NULL"
                        if not q.air_date
                        else f"'{str(q.air_date).replace(chr(39), chr(39) + chr(39))}'"
                    )
                    round_type_val = (
                        "NULL"
                        if not q.round_type
                        else f"'{str(q.round_type).replace(chr(39), chr(39) + chr(39))}'"
                    )
                    value_val = "NULL" if not q.value else str(q.value)
                    show_number_val = (
                        "NULL" if not q.show_number else str(q.show_number)
                    )
                ...
                for u in usage_records:
                    f.write(
                        f"INSERT INTO turnover_usage (user_id, last_used_date, usage_count) VALUES ("
                    )
            ...
            logger.info(
                f"Backup contains: {len(questions)} questions, {len(usage_records)} usage records"
            )
    ...
    def create_pg_dump_backup(self, filename: Optional[str] = None) -> str:
        ...
        try:
            ...
            logger.warning(
                "pg_dump command not found. Using SQL backup method instead."
            )
            return self.create_sql_backup(filename)
    ...
    def create_json_backup(self, filename: Optional[str] = None) -> str:
        ...
        try:
                ...
                backup_data["turnover_usage"].append(
                    {
                        ...
                        if hasattr(u, "updated_at") and u.updated_at is not None
                        else None,
                    }
                )
            ...
            logger.info(
                f"Backup contains: {len(backup_data['jeopardy_questions'])} questions, "
        ...
        if args.action == "backup":
            ...
            if not backups:
                ...
                for backup in backups:
                    ...
                    print(
                        f"{backup['filename']:<40} {backup['type']:<4} "
