...
class DiscordBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        if len(self.guilds) == 0:
            logger.warning(
                "Bot is not in any guilds. Make sure the bot is properly invited to a server."
            )
            logger.info(
                "Bot invite URL should include 'bot' and 'applications.commands' scopes"
            )
    ...
    async def on_guild_join(self, guild):
        ...
        if guild.system_channel:
            try:
                ...
                logger.warning(
                    f"No permission to send message in {guild.name} system channel"
                )
